...
import { getSuggestFriends } from "@lib/getSuggestFriends";
...
interface SuggestFriendsListProps {
  userId: string;
}

const SuggestFriendsList = ({ userId }: SuggestFriendsListProps) => {
  const [friends, setFriends] = useState<SuggestFriend[]>([]);
  ...
  useEffect(() => {
    const fetchYourSuggestFriends = async () => {
      try {
        const response = await getSuggestFriends(userId);
        setFriends(response);
        setIsLoading(false);
      } catch (err) {
...
export default SuggestFriendsList;
