...
func Register(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil { // If a file is uploaded
		...
		// Set the full URL for the avatar
		req.Avatar = fmt.Sprintf("http://localhost:8000/uploads/%s", filepath.Base(avatarPath))
	}
...
func saveAvatar(file io.Reader) (string, error) {
	...
	log.Printf("Avatar saved at: %s", path)

	return "/uploads/" + filename, nil // Return relative path
}

func Logout(w http.ResponseWriter, r *http.Request) {
	// Clear the session cookie
	middlewares.ClearSessionCookie(w)
